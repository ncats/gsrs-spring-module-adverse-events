package gov.hhs.gsrs.adverseevents.adverseeventdme.controllers;

import gov.hhs.gsrs.adverseevents.adverseeventdme.models.*;
import gov.hhs.gsrs.adverseevents.adverseeventdme.services.*;
import gov.hhs.gsrs.adverseevents.adverseeventdme.searcher.*;
...
public class AdverseEventDmeController extends EtagLegacySearchEntityController<AdverseEventDmeController, AdverseEventDme, Long> {
    ...
    @Autowired
    private AdverseEventDmeEntityService adverseEventEntityService;

    @Autowired
    private LegacyAdverseEventDmeSearcher legacyAdverseEventDmeSearcher;
    ...
    @Override
    protected LegacyGsrsSearchService<AdverseEventDme> getlegacyGsrsSearchService() {
        return legacyAdverseEventDmeSearcher;
    }

    @Override
    protected Stream<AdverseEventDme> filterStream(Stream<AdverseEventDme> stream, boolean publicOnly, Map<String, String> parameters) {
        return stream;
    }
...
//        }
        ...
        Stream<AdverseEventDme> effectivelyFinalStream = filterStream(mstream, publicOnly, parameters);

        if(fileName!=null){
            emd.setDisplayFilename(fileName);
        }

        ExportProcess<AdverseEventDme> p = exportService.createExport(emd,
    ...
    private Exporter<AdverseEventDme> getExporterFor(String extension, OutputStream pos, boolean publicOnly, Map<String, String> parameters) throws IOException {
        ExporterFactory.Parameters params = this.createParamters(extension, publicOnly, parameters);
        ExporterFactory<AdverseEventDme> factory = this.gsrsExportConfiguration.getExporterFor(this.getEntityService().getContext(), params);
        if (factory == null) {
